...
export default function LoginMedico() {
  ...
  async function fazerCadastro(dados) {
    ...
    // O cadastro já salva os procedimentos no Back-End.
    // Não é necessário chamar uma rota protegida.
    setCadastro(false);
  }

  return (
    <main className="auth-page">
      <section className="auth-container wide">
        <div className="brand">
        ...
        <h1>
          {cadastro ? 'Cadastro de médico' : 'Login do médico'}
        </h1>

        <p className="muted">
          {cadastro
            ? 'Preencha seus dados profissionais.'
            : 'Entre para acessar a área médica.'}
        </p>
        ...
        {cadastro ? (
          <CadastroMedicoForm
            ...
            loading={loading}
            onVoltar={() => setCadastro(false)}
          />
        ) : (
          <LoginMedicoForm
            onSubmit={fazerLogin}
            loading={loading}
            onCadastrar={() => setCadastro(true)}
          />
        )}

        <Link className="doctor-link" to="/">
